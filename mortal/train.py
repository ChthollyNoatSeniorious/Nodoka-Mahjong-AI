def train():
    ...
    def train_epoch():
        ...
        # Always persist the true end-of-epoch state. Without this, up to
        # `save_every` steps of progress are silently dropped, because the only
        # other save point is `steps % save_every == 0` inside train_batch().
        torch.save({
            'mortal': mortal.state_dict(),
            'current_dqn': dqn.state_dict(),
            'aux_net': aux_net.state_dict(),
        ...
        logging.info(f'saved end-of-epoch state at step {steps:,}')
